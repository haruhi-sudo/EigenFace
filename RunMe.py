...
if __name__ == '__main__':
    ...
    for train_id in range(k):
        ...
        for i in range(len(all_labels)):
            if i == 0 or all_labels[i] != all_labels[i-1]:
                image_dic[all_labels[i]] = [train_images[i]]
            else:
                image_dic[all_labels[i]].append(train_images[i])
        ...
        predict_result, test_labels, files = predict(test_path + str(train_id), eigen_dic, meanface_dic)
